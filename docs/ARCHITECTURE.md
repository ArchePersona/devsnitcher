...
DEVPEEPER does nothing until an explicit **SNITCH** press. There is NO auto-attach: extension start, tab activation, navigation and popup-open never attach the debugger. SNITCH is a DevTools evidence snapshot: acquisition runs to completion when the actual reads finish, with no six-second (or any) harvest window. The user-facing lifecycle is `IDLE → SNITCH → SNITCHSHOT_PENDING → COPY SNITCHSHOT → clipboard release → IDLE`; `OBSERVING` (internal) means "SNITCH acquisition is actively executing", and `OBSERVING → CANCEL → IDLE` terminates it.

1. User opens the popup, optionally enters a short description, and clicks **SNITCH** on the tab they want snapshotted.
2. Popup sends `SNITCH` to the background service worker.
3. Background refuses tab-relayed `SNITCH` messages, then checks two gates: no other acquisition is already running, and no SNITCHSHOT is already pending. If acquisition is live, SNITCH is refused; if a SNITCHSHOT is pending, SNITCH is refused until it is copied out.
...
5. DEVPEEPER's bounded probe captures environment, focused DOM and current selection on that session tab through `chrome.scripting.executeScript`, returned in Chrome's `InjectionResult`. This is the on-demand snapshot of the page's current diagnostic state.
6. Console, runtime-error and network evidence is retrospective-vs-prospective by Chrome's boundary: `Runtime`/`Network` domains only emit events after attachment, so the observer retains what Chrome emits while attached (e.g. an in-flight failure that completes during acquisition) and never invents history from before SNITCH. None of the surfaces is followed by a harvest wait — the acquisition finishes when the reads finish.
7. When acquisition is complete, background finalizes: assembles the evidence, stops the observer (detaches the debugger), applies redaction, and builds the report.
8. Background stores the completed report as the outstanding SNITCHSHOT in the private buffer (occupied), then verifies retrieval (a pending SNITCHSHOT is never advertised unless the report is retrievable). A second SNITCH while occupied is refused from every tab.
...
11. While acquisition is still active, **CANCEL** terminates it: it detaches the debugger, discards the unfinished acquisition, and the session never resurrects. Once the SNITCHSHOT is produced, CANCEL is no longer the valid action.
...
Only one DEVPEEPER observer exists at a time, and only during a SNITCH acquisition. Nothing attaches before SNITCH: extension start, `chrome.tabs.onActivated`, navigation and popup-open never trigger a debugger attachment. The observer is permanently bound to the tab that was selected at SNITCH time — switching tabs never moves, restarts or resurrects the session. While attached, the observer retains bounded console (200), runtime-error (50) and network (100) history from events Chrome actually emits during the acquisition (these surfaces are prospective-only: CDP `Runtime`/`Network` domains do not replay history, so they cannot contain anything from before SNITCH and are never fabricated). The acquisition completes when its reads finish — nothing waits for a harvest window. `CANCEL` is terminal while acquisition is active: it detaches and discards, and no later tab event can resurrect the session.
...
- **System clipboard release.** The popup's primary action becomes **COPY SNITCHSHOT**. The popup pre-fetches the pending report (non-tab sender only) into its own memory, writes it to the Windows OS clipboard with the popup-document DOM copy path (`document.execCommand('copy')` against a focused, fully selected offscreen `<textarea>`), then sends `CLIPBOARD_RELEASED`. Background verifies the private buffer actually cleared after removal and only then answers `CLIPBOARD_CLEARED`; a failed/unconfirmed clear keeps the SNITCHSHOT pending with an error. The page never messages the extension to trigger a copy; tab-relayed `GET_SNITCHSHOT`/`CLIPBOARD_RELEASED` are refused.
- **Consumption boundary.** The buffer is cleared only after a confirmed OS clipboard write and a confirmed buffer clear. A failed write retains the buffer so the user can press COPY again. A report cleared only on confirmed delivery means "a SNITCHSHOT was copied to the clipboard" cannot be mistaken for a silent discard. The async `navigator.clipboard` API is intentionally never the text-release authority: it is unreliable from a popup document (it can resolve or reject without a focused document), so only the synchronous DOM copy path handles the ordinary text CTA.
