...
DEVPEEPER does not follow the tab. There is **no** `chrome.tabs.onActivated` auto-attach: extension start, tab activation, navigation and popup-open never attach the debugger. Only an explicit **SNITCH** press starts a session on the tab the user selected; that session tab is immutable — switching tabs never moves, restarts or resurrects the session. The session completes when its surfaces are complete: bounded context once acquired, and each of console/jsErrors/network when they have entries **OR** a bounded harvest window elapses (empty is legitimate and complete), so a session never waits indefinitely for an error, then detaches. `CANCEL` is terminal and the session never resurrects. Browser-internal/unsupported pages are excluded.
